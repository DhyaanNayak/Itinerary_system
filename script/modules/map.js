const GM_STYLES = {
  ...
  icons: {
    ...
    museum:
      "data:image/png;base64,",
    hotel:
...
function getPlaceIntro(result, type) {
  let introHTML = `<div class="place-intro"><div class="icon icon-${type}"><i class="material-icons">local_${type}</i></div><p class="place-name">${result.name}</p></div><p class="place-address">${result.formatted_address}</p><div class="place-contact"><p><i class="material-icons">local_phone</i> ${result.formatted_phone_number}</p><p><i class="material-icons">language</i><a href="${result.website}">${result.website}</a></p>`;
  if (result.opening_hours && result.opening_hours.weekday_text) {
    introHTML += `<div class="place-hours"><h4>Business Hours</h4>`;
    result.opening_hours.weekday_text.forEach((hour) => {
      introHTML += `<p>${hour}</p>`;
    });
    introHTML += `</div>`;
  }
...
//******  A* Algorithm to  find the shortest route ************//
...
// Define a function to calculate the heuristic (straight-line distance) between two points
...
class Node {
  constructor(placeId, location) {
    this.placeId = placeId;
    this.location = location;
    this.gScore = Infinity; // The cost of getting from the start node to this node
    this.fScore = Infinity; // The total cost of getting from the start node to the goal by passing by this node
    this.previous = null; // Pointer to the previous node in the optimal path
  }
}

// Implement the A* algorithm
function astar(start, goal, waypoints) {
  const openSet = new Set(); // The set of nodes to be evaluated
  const closedSet = new Set(); // The set of nodes already evaluated

  // Add the start node to the open set
  start.gScore = 0;
  start.fScore = calculateHeuristic(start.location, goal.location);
  openSet.add(start);
  ...
  while (openSet.size > 0) {
    ...
    for (const waypoint of waypoints) {
      ...
      const tentativeGScore =
        current.gScore +
        calculateDistance(current.location.lat, current.location.lng, neighbor.location.lat, neighbor.location.lng);

      // If the tentative gScore is lower than the neighbor's gScore
      if (tentativeGScore < neighbor.gScore) {
        // This path to the neighbor is better than any previous one. Record it!
        neighbor.previous = current;
        neighbor.gScore = tentativeGScore;
        neighbor.fScore = neighbor.gScore + calculateHeuristic(neighbor.location, goal.location);
  ...
  // If the open set is empty but the goal was never reached, return null (no path found)
  return null;
}
