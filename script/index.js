...
// Listen for place change in google searchplace
const searchplace = new google.maps.places.SearchBox(elements.placeInput);
searchplace.addListener("places_changed", () => {
  const place = searchplace.getPlaces()[0];
  if (place === null) return;
  state.placeName = place.name;
  state.mapCenter = place.geometry.location;
});
...
const control= async (e) => {
  try {
    ...
    // Mark the specific location provided by the user
    map.savePlace(place, place);
    map.addMarker(place);
    map.showMarkers(place);
    map.createCard(place);

    // Call weather forecast from API for the selected location and render weather info in daily planners
